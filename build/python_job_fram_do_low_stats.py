...
#REC_POS_list=           [0]
REC_POS_list = [2,2,3,4,4,5,6,6,7,8,8]
#REC_POS_list = [3]
#REC_POS_list = [2]
#Efield_alt_center=      [16]
#Efield_alt_center_list = [12]
Efield_alt_center_list = [10,12,12,8,10,10,6,8,8,6,12]
...
# random.shuffle(Efield_alt_center_list)
# random.shuffle(REC_POS_list)

MAX_POT_FOR_CENTER_ALT_LIST = [310, 250, 200, 160, 151, 151, 121]
    ...
    for ii, _ in enumerate(Efield_alt_center_list):
        for kk, _ in enumerate(Efield_full_size_list):
            for POT in POTENTIAL_LIST:
                ...
                rec_alt = Efield_alt_center_list[ii] + REC_POS_list[ii]*Efield_full_size_list[kk]/2.0

                if rec_alt not in [12, 14, 15, 20]:
                    continue

                #if abs(POT) > MAX_POT_FOR_CENTER_ALT_LIST[ii]:
                #    continue
