...
splini(double * xa, double * ya, double * y2a, int n, double x, double * y)
{
    ...
    while ((x > xa[klo]) && (khi < n)) {
        ...
        yi +=
          ...
          + ((-(1.0 + a2 * a2) / 4.0 + a2 / 2.0) * y2a[klo] + (b2 * b2 / 4.0 - b2 / 2.0) * y2a[khi]) * h * h / 6.0) * h;
        klo++;
        khi++;
    }
    *y = yi;
...
sg0(double ex, double * p, double * ap)
{
    return (g0(ap[1],
           ...
           p)
           * pow(ex,
           3.0) + (g0(ap[5], p) * pow(ex, 4.0) + g0(ap[6], p) * pow(ex, 12.0)) * (1.0 - pow(ex, 8.0)) / (1.0 - ex)))
           / sumex(ex);
}
        ...
        if (flags->sw[13]) {
            if (flags->sw[9] == -1) {
                ...
                t[12] = apdf * flags->swc[11] * (1.0 + p[120] * plg[0][1])
                  * ((p[60] * plg[1][2] + p[61] * plg[1][4] + p[62] * plg[1][6]) * cos(dgtr * (input->g_long - p[63])))
...
glob7s(double * p, struct nrlmsise_input * input, struct nrlmsise_flags * flags)
{
    ...
    if (flags->sw[7]) {
        ...
        t72  = p[12] * plg[1][2] * cd14 * flags->swc[5];
        t[6] =
          ((p[3] * plg[1][1] + p[4] * plg[1][3] + t71) * ctloc + (p[6] * plg[1][1] + p[7] * plg[1][3] + t72) * stloc);
    }
    ...
    if (!((flags->sw[10] == 0) || (flags->sw[11] == 0) || (input->g_long <= -1000.0))) {
        t[10] =
          ...
          * ((p[64] * plg[1][2] + p[65] * plg[1][4] + p[66] * plg[1][6] + p[74] * plg[1][1] + p[75] * plg[1][3]
          ...
          + (p[90] * plg[1][2] + p[91] * plg[1][4] + p[92] * plg[1][6] + p[77] * plg[1][1] + p[78] * plg[1][3] + p[79]
...
gtd7(struct nrlmsise_input * input, struct nrlmsise_flags * flags, struct nrlmsise_output * output)
{
    ...
    meso_tgn2[1] = pavgm[8] * pma[9][0]
      ...
      * glob7s(pma[9], input, flags)) * meso_tn2[3] * meso_tn2[3] / (pow((pma[2][0] * pavgm[2]), 2.0));
    ...
    if (input->alt < zn3[0]) {
        ...
        meso_tgn3[1] = pma[7][0] * pavgm[7]
          * (1.0 + flags->sw[22]
          * glob7s(pma[7], input, flags)) * meso_tn3[4] * meso_tn3[4] / (pow((pma[6][0] * pavgm[6]), 2.0));
    }
    ...
    /**** Total mass density */
    output->d[5] = 1.66E-24
      * (4.0 * output->d[0] + 16.0 * output->d[1] + 28.0 * output->d[2] + 32.0 * output->d[3] + 40.0 * output->d[4]
...
gts7(struct nrlmsise_input * input, struct nrlmsise_flags * flags, struct nrlmsise_output * output)
{
    ...
    if (input->alt < 300.0) {
        ...
        meso_tgn1[1] = ptm[8] * pma[8][0]
          ...
          * glob7s(pma[8], input, flags)) * meso_tn1[4] * meso_tn1[4] / (pow((ptm[4] * ptl[3][0]), 2.0));
    } else {
