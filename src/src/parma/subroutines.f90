...
! *******************************************************
    ...
    if(B(2).eq.0.0d0) then ! first time
    ...
    if(B(4).eq.0.0d0) then ! first time
    ...
    if(down.eq.0.0d0) then ! first time call this routine
     down=1.720313d0
     do ig=1,ngroup
    ...
    real*8 :: getr, cor1, cor2
    real*8, intent(in) ::cido,ckei
    ...
    ! ang: cos(theta)
    implicit real*8 (a-h, o-z)
    if(ip.eq.4.and.g.ge.0.0d0.and.g.le.1.0d0) then ! ground level muon
    ...
    do is1=is-1,is
     do ic1=ic-1,ic
      idx=idx+1
      do i=1,maxfit
       if(e.eq.0.0d0) then ! energy integrated
    ...
    if(ip.eq.4.and.d.gt.60.and.e.ge.7000.0) then ! for debugging
     continue
    endif
    ...
    if(FFP(iy0,im0,id0).gt.-99.0) then ! data exist
     if(iy0.ge.iystart.and.FFP(iy0,im0,id0).eq.-1000.0d0) then ! no such date
      ic=5
      getHP=0.0
