...
mission = 'ADELE2015';
WANTED_REC_ALT = 14;
MAX_WANTED_INCREASE = 10.;
MIN_WANTED_INCREASE = 1.2;
    ...
    for i_efield_c = 1 : length(sett.EFIELD_CENTER_list)
        for i_efield_s = 2
            for i_pot = 1:length(sett.POTENTIAL_LIST)
                ...
                if APPLY_RESP
                    increase_list(end+1) = sum(detection_factor.*[1,0.1,0.1])/sum(detection_factor_bg.*[1,0.1,0.1]);
                else
...
path_to_folder = '/Home/siv29/dsa030/Desktop/article_cr_modeling/DATA_REPO/ARTICLE_DATA/Figure_6/';
saveas(gcf,[path_to_folder mission '.eps'],'epsc')
saveas(gcf,[path_to_folder mission '.png'],'png')
