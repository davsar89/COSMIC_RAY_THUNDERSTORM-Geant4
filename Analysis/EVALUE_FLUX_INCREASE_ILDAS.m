...
mission = 'ILDAS2017';
WANTED_REC_ALT = 12;
MAX_WANTED_INCREASE = 20.0;
MIN_WANTED_INCREASE = 3;
YLIM_PLOT = [9.0 16.0];
XLIM_PLOT = [-222 222];
...
increase_due_to_positrons_for_plot=zeros(length(sett.EFIELD_CENTER_list),length(sett.POTENTIAL_LIST));
    ...
    for i_efield_c = 1 : length(sett.EFIELD_CENTER_list)
        for i_efield_s = 2
            for i_pot = 1:length(sett.POTENTIAL_LIST)
                ...
                %                 if increase_due_to_positrons>2
                %                      disp(['POTENTIAL: ' num2str(POTENTIAL)])
                %                     disp(['EFIELD magnitude: ' num2str(POTENTIAL/EFIELD_SIZE) ' MV/km'])
                %                     disp(['REC POS: ' num2str(RECORD_POS)])
                %                     disp(['EFIELD CENTER: ' num2str(ALT)])
                ...
                if rec_alt==WANTED_REC_ALT
                    ...
                    increase_due_to_positrons_for_plot(i_efield_c,i_pot) = increase_due_to_positrons/(increase_due_to_photons+increase_due_to_electrons+increase_due_to_positrons);
...
path_to_folder = '/Home/siv29/dsa030/Desktop/article_cr_modeling/DATA_REPO/ARTICLE_DATA/Figure_7/';
saveas(gcf,[path_to_folder mission '.eps'],'epsc')
saveas(gcf,[path_to_folder mission '.png'],'png')
